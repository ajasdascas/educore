...
CREATE TABLE IF NOT EXISTS kinder_daily_logs (
  ...
  general_mood ENUM('happy','calm','sad','anxious','tired','energetic','irritable') NULL,
  notes       TEXT         NULL,
...
CREATE TABLE IF NOT EXISTS kinder_meals (
  ...
  meal_time   ENUM('breakfast','morning_snack','lunch','afternoon_snack') NOT NULL DEFAULT 'lunch',
  portion     ENUM('full','half','little','none') NOT NULL DEFAULT 'full',
...
CREATE TABLE IF NOT EXISTS kinder_naps (
  ...
  quality          ENUM('good','restless','none') NOT NULL DEFAULT 'good',
...
CREATE TABLE IF NOT EXISTS kinder_diapers (
  ...
  diaper_type  ENUM('wet','soiled','both','dry') NOT NULL DEFAULT 'wet',
  notes        VARCHAR(300) NULL,
  created_at   DATETIME  NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS kinder_mood (
  ...
  mood_code   ENUM('happy','calm','sad','anxious','tired','energetic','irritable') NOT NULL DEFAULT 'calm',
  notes       VARCHAR(300) NULL,
  created_at  DATETIME  NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS kinder_incidents (
  ...
  incident_type    ENUM('fall','bite','scratch','illness','allergy','other') NOT NULL DEFAULT 'other',
...
CREATE TABLE IF NOT EXISTS preschool_qualitative_assessments (
  ...
  nivel               ENUM('logrado','en_proceso','iniciando','requiere_apoyo') NOT NULL DEFAULT 'en_proceso',
  notes               TEXT         NULL,
  created_at          DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at          DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS preschool_observations (
  ...
  category             ENUM('general','conduct','academic','social','physical') NOT NULL DEFAULT 'general',
