...
var secretLikePattern = regexp.MustCompile(`(?i)(secret|token|password|passwd|pwd|mysql_dsn|database_url|jwt|stripe_secret|api[_-]?key)\s*[:=]`)
...
func normalizeDeploymentRequest(req deploymentRecordRequest) deploymentRecordRequest {
	...
	req.Title = redactSecretLikeText(truncateDeploymentField(req.Title, 255))
