...
export default function ParentNotificationsPage() {
  ...
  const markRead = async (id: string) => {
    setMarkingId(id);
    try {
      const res = await authFetch(`/api/v1/parent/notifications/${id}/read`, { method: "PUT" });
      if (res?.success) {
  ...
  const markAllRead = async () => {
    ...
    await Promise.allSettled(unread.map((n) => authFetch(`/api/v1/parent/notifications/${n.id}/read`, { method: "PUT" })));
              ...
              {notifications.map((n) => (
                <div key={n.id} className={`flex items-start justify-between gap-3 py-4 ${n.is_read ? "opacity-70" : ""}`}>
                  <div className="min-w-0 flex-1">
                    ...
                    {(n.message || n.body) && (
                      <p className="mt-0.5 text-xs text-muted-foreground line-clamp-2">{n.message || n.body}</p>
                    )}
