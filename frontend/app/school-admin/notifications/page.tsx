...
export default function SchoolAdminNotificationsPage() {
  ...
  useEffect(() => {
    authFetch("/api/v1/school-admin/notifications")
      .then((res) => {
        if (res?.success) {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Avisos del sistema y de la plataforma.</p>
      </div>
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base">
            <Bell className="h-4 w-4" /> Centro de notificaciones
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="flex h-32 flex-col items-center justify-center gap-2 text-muted-foreground">
              ...
              <p className="text-sm">No hay notificaciones por ahora.</p>
            </div>
          ) : (
