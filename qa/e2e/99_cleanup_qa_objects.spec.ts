...
test('cleanup: solo objetos QA-CODEX creados por esta corrida', async () => {
  ...
  const unsafe = created.filter((item: any) => !String(item.name || item.slug || item.id || '').includes('QA-CODEX'));
  ...
  if (unsafe.length) {
    recordResult({
      ...
      expected: 'Todo objeto creado por la auditoria contiene QA-CODEX.',
  ...
  recordResult({
    ...
    expected: 'Archivar/eliminar solo objetos QA-CODEX con API documentada.',
